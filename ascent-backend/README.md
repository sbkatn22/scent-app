...
- [Fragrance API](#fragrance-api-apifragrances)
  ...
  - [Toggle wishlist (POST)](#toggle-wishlist-post)
  - [Get wishlist (GET)](#get-wishlist-get)
  - [Toggle fragrance like (POST)](#toggle-fragrance-like-post)
  - [Get liked fragrances (GET)](#get-liked-fragrances-get)
...
- [Reviews API](#reviews-api-apireviews)
  ...
  - [Toggle review like (POST)](#toggle-review-like-post)
  - [Comments](#comments)
    - [List comments for a review (GET)](#list-comments-for-a-review-get)
    - [Create comment or reply (POST)](#create-comment-or-reply-post)
    - [Update comment (PATCH)](#update-comment-patch)
    - [Delete comment (DELETE)](#delete-comment-delete)
    - [Toggle comment like (POST)](#toggle-comment-like-post)
...
Fragrance endpoints for searching/listing perfumes, creating new ones, collections, wishlists, likes, and daily scents. Request bodies are JSON where applicable; responses are JSON.
...
> Note: `count` is the number of items returned in `results` for that page (not the total number of matches).
...
Returns the created fragrance in the same shape as a single item in the search `results` array.
...
{
  "collection": [
    { "id": 1, "size": "BOTTLE", "added_on": "2026-02-28T12:00:00Z", /* ...fragrance fields */ }
  ]
}
```
...
Add a fragrance to the authenticated user's wishlist or remove it if already present. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Body | Success | Error |
|--------|------|---------|------|---------|-------|
| POST | `/api/fragrances/wishlist/toggle/` | `Authorization: Bearer <access_token>` | `{ "perfume_id": <int> }` | `200` removed / `201` added | `400` missing perfume_id; `401` invalid token; `404` perfume not found |

#### Example request

```http
POST /api/fragrances/wishlist/toggle/
Authorization: Bearer <access_token>
Content-Type: application/json
...
```json
{ "message": "Perfume removed from wishlist", "perfume_id": 123 }
```

#### Success response (201 — added)

```json
{ "message": "Perfume added to wishlist", "perfume_id": 123 }
```
...
Returns the authenticated user's wishlist. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Success | Error |
|--------|------|---------|---------|-------|
| GET | `/api/fragrances/wishlist/get/` | `Authorization: Bearer <access_token>` | `200` — `{ "wishlist" }` | `401` invalid token |
...
{
  "wishlist": [
    { "id": 1, "fragrance": "...", "brand": "...", /* ...fragrance fields */ }
  ]
}
```
...
### Toggle fragrance like (POST)

Like or unlike a fragrance. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Body | Success | Error |
|--------|------|---------|------|---------|-------|
| POST | `/api/fragrances/likes/toggle/` | `Authorization: Bearer <access_token>` | `{ "perfume_id": <int> }` | `200` unliked / `201` liked | `400` missing perfume_id; `401` invalid token; `404` perfume not found |

#### Example request

```http
POST /api/fragrances/likes/toggle/
Authorization: Bearer <access_token>
Content-Type: application/json
...
```json
{ "message": "Fragrance unliked", "perfume_id": 123, "liked": false }
```

#### Success response (201 — liked)

```json
{ "message": "Fragrance liked", "perfume_id": 123, "liked": true }
```

---

### Get liked fragrances (GET)

Returns all fragrances the authenticated user has liked. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Success | Error |
|--------|------|---------|---------|-------|
| GET | `/api/fragrances/likes/get/` | `Authorization: Bearer <access_token>` | `200` — `{ "liked_fragrances" }` | `401` invalid token |
...
Creates a daily scent entry for the authenticated user (stores which perfume they wore on a given day). Requires `Authorization: Bearer <access_token>`. Uses Upstash Redis for storage with DB fallback.
...
| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `perfume_id` | integer | No | ID of the perfume. Omit or send `null` to clear the day's scent. |
...
When clearing a day's scent (`perfume_id` omitted):
...
### Get daily scents (GET — all)

Returns all daily scent entries for the authenticated user. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Success | Error |
|--------|------|---------|---------|-------|
| GET | `/api/fragrances/daily_scent/get/all/` | `Authorization: Bearer <access_token>` | `200` — `{ "daily_scents" }` | `401` invalid token |
...
|--------|------|---------|--------------|---------|-------|
| GET | `/api/fragrances/daily_scent/get/` | `Authorization: Bearer <access_token>` | `timestamp` (required) | `200` — `{ "daily_scent": <fragrance> }` | `401` invalid token; `500` parse error |
...
Review endpoints for creating and listing user reviews of fragrances, plus comments and likes. Request bodies are JSON; responses are JSON.
...
| `uid` | UUID (string) | Supabase UID of the reviewer. |
| `username` | string | Username of the reviewer. |
| `profile_picture` | string | Profile picture URL of the reviewer (empty string if none). |
| `fid` | integer | Fragrance ID (`Perfume.id`). |
...
| `longevity` | string | `"0 - 2 hours"`, `"2 - 4 hours"`, `"4 - 6 hours"`, `"6 - 8 hours"`, `"8-10 hours"`, `"10+ hours"`. |
| `value` | string | `"Super Overpriced"`, `"Overpriced"`, `"Alright"`, `"Good Value"`, `"Super Value"`. |
| `maceration` | integer or null | Optional days for maceration. |
| `like_count` | integer | Total number of likes on this review. |
| `liked` | boolean | Whether the requesting user has liked this review (`false` if unauthenticated). |
...
Creates a new review for a fragrance. One review per user per fragrance is enforced. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Body | Success | Error |
|--------|------|---------|------|---------|-------|
| POST | `/api/reviews/create/` | `Authorization: Bearer <access_token>` | JSON (see below) | `201` — review object | `400` validation error; `401` invalid token; `404` perfume not found; `409` already reviewed |
...
| Key | Type | Description |
|-----|------|-------------|
| `fid` | integer | ID of the perfume being reviewed. |
| `description` | string | Review text; 1–500 characters. |
...
Returns the created review object (see [Review object](#review-object)).
...
Partially updates an existing review. Only the review's author may edit it. All fields are optional.
...
Same fields as create (`description`, `rating`, `gender`, `longevity`, `value`, `winter`, `spring`, `summer`, `autumn`, `day`, `night`, `maceration`). Only supplied fields are updated.

#### Success response (200)

Returns the full updated review object.
...
Returns all reviews for a given perfume. Auth is optional — if a valid Bearer token is provided, `liked` reflects whether that user has liked each review.

| Method | Path | Query params | Headers | Success | Error |
|--------|------|-------------|---------|---------|-------|
| GET | `/api/reviews/` | `fid` (required) | `Authorization: Bearer <access_token>` (optional) | `200` — `{ "count", "results" }` | `400` missing/invalid fid |
...
Same response shape as **List reviews for a fragrance**.

---

### Toggle review like (POST)

Like or unlike a review. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Body | Success | Error |
|--------|------|---------|------|---------|-------|
| POST | `/api/reviews/likes/toggle/` | `Authorization: Bearer <access_token>` | `{ "review_id": <int> }` | `200` — `{ "message", "review_id", "like_count", "liked" }` | `400` missing review_id; `401` invalid token; `404` review not found |

#### Example request

```http
POST /api/reviews/likes/toggle/
Authorization: Bearer <access_token>
Content-Type: application/json
...
```json
{ "message": "Review liked", "review_id": 1, "like_count": 6, "liked": true }
```
...
Comments belong to a review. Replies are comments with a `parent_id` set. Replies can themselves have replies (unlimited nesting). The GET endpoint returns the full nested tree. Auth is optional for fetching — `liked` is `false` if unauthenticated.
...
| `review_id` | integer | The review this comment belongs to. |
| `parent_id` | integer or null | ID of the parent comment (`null` for top-level). |
| `content` | string | Comment text; max 1000 characters. |
| `author` | object | `{ "uid", "username", "profile_picture" }` |
| `like_count` | integer | Total likes on this comment. |
| `liked` | boolean | Whether the requesting user has liked this comment. |
| `created_at` | ISO 8601 datetime | — |
| `updated_at` | ISO 8601 datetime | — |
| `replies` | array | Nested comment objects (same shape, recursive). |

---

### List comments for a review (GET)

Returns all comments for a review as a nested tree. Auth is optional.

| Method | Path | Query params | Headers | Success | Error |
|--------|------|-------------|---------|---------|-------|
| GET | `/api/reviews/comments/` | `review_id` (required) | `Authorization: Bearer <access_token>` (optional) | `200` — `{ "comments" }` | `400` missing/invalid review_id; `404` review not found |

#### Example request

```http
GET /api/reviews/comments/?review_id=1
Authorization: Bearer <access_token>
```
  ...
  "comments": [
    {
      ...
      "author": { "uid": "...", "username": "alice", "profile_picture": "" },
      "like_count": 3,
      "liked": false,
      ...
      "replies": [
        {
          ...
          "parent_id": 1,
          "content": "Agreed!",
          "author": { "uid": "...", "username": "bob", "profile_picture": "" },
          "like_count": 1,
          "liked": true,
...
### Create comment or reply (POST)

Creates a top-level comment on a review, or a reply to an existing comment. Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Body | Success | Error |
|--------|------|---------|------|---------|-------|
| POST | `/api/reviews/comments/create/` | `Authorization: Bearer <access_token>` | JSON (see below) | `201` — comment object | `400` missing fields / content too long; `401` invalid token; `404` review or parent not found |

#### Request body

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `review_id` | integer | Yes | ID of the review being commented on. |
| `content` | string | Yes | Comment text; 1–1000 characters. |
| `parent_id` | integer | No | ID of the comment being replied to. Omit for a top-level comment. |

#### Example request

```http
POST /api/reviews/comments/create/
Authorization: Bearer <access_token>
Content-Type: application/json

{ "review_id": 1, "content": "Great review!", "parent_id": null }
```

#### Success response (201)

Returns the created comment object (without nested `replies`, since it is new).

---

### Update comment (PATCH)

Updates the content of a comment. Only the comment's author may edit it.

| Method | Path | Headers | Body | Success | Error |
|--------|------|---------|------|---------|-------|
| PATCH | `/api/reviews/comments/update/<comment_id>/` | `Authorization: Bearer <access_token>` | `{ "content": "<string>" }` | `200` — updated comment object | `400` empty/too-long content; `401` invalid token; `403` not the author; `404` comment not found |

---

### Delete comment (DELETE)

Deletes a comment. Only the comment's author may delete it. Deleting a comment also deletes all of its replies (cascade).

| Method | Path | Headers | Success | Error |
|--------|------|---------|---------|-------|
| DELETE | `/api/reviews/comments/delete/<comment_id>/` | `Authorization: Bearer <access_token>` | `204` — empty body | `401` invalid token; `403` not the author; `404` comment not found |

---

### Toggle comment like (POST)

Like or unlike a comment (or reply). Requires `Authorization: Bearer <access_token>`.

| Method | Path | Headers | Body | Success | Error |
|--------|------|---------|------|---------|-------|
| POST | `/api/reviews/comments/likes/toggle/` | `Authorization: Bearer <access_token>` | `{ "comment_id": <int> }` | `200` — `{ "message", "comment_id", "like_count", "liked" }` | `400` missing comment_id; `401` invalid token; `404` comment not found |

#### Example request

```http
POST /api/reviews/comments/likes/toggle/
Authorization: Bearer <access_token>
Content-Type: application/json
...
```json
{ "message": "Comment liked", "comment_id": 2, "like_count": 2, "liked": true }
```
...
Creates the user in **Supabase Auth**, then creates a local **Profile** using the returned UID.
...
| Key | Type |
|-----|------|
| `username` | string (non-empty if present) |
| `bio` | string |
| `profile_picture` | string |
...
- **400 Bad Request** — Missing or invalid JSON; missing required fields; blank `username`; username already exists; review validation (rating range, description length, invalid gender/longevity/value choices); cannot follow self; comment content empty or too long.
- **401 Unauthorized** — Missing, invalid, or expired Bearer token; invalid refresh token; invalid credentials.
- **403 Forbidden** — Authenticated user is not the author of the review or comment they are trying to update or delete.
- **404 Not Found** — No profile for the authenticated user; profile not found for follow `uid`; perfume not found; review not found; comment or parent comment not found.
